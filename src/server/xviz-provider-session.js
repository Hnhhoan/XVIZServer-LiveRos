...
import { XVIZLiveProviderRequestHandler } from '../middlewares/xviz-live-provider-request-handler';
...
import { LiveRosDataProvider } from '../xviz-live-data-provider/live-ros-data-provider';
...
export class XVIZProviderSession {
  ...
  _setupMiddleware() {
    ...
    const liveProvider=new LiveRosDataProvider(options);
    const stack = [
      new XVIZLiveProviderRequestHandler(this.context, liveProvider, this.middleware, options),
  ...
  onConnect() {
    ...
    //console.log("params from onConnect method, XVIZ-Provider-Session");
    //console.log(params);
