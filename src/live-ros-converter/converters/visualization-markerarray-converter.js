 ...
 */
export default class VisualizationMarkerArray {
  constructor(topicConfig) {
    ...
    this.topicConfig = topicConfig;// topic config
    this.xvizStream=this.topicConfig.config.xvizStream
    this.acceptMarker = this.topicConfig.config.acceptMarker || (() => true);
  ...
  convertMessage(synchronizedData, xvizBuilder) {
    const messages = synchronizedData.get(this.topicConfig.topic);
    //console.log(synchronizedData.get(this.topicConfig.topic).markers.length);
    if (messages) {
      messages.markers.forEach(marker => this._processMarker(marker));
    }else{
      return;
    }
